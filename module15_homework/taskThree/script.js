...
function pageLoaded() {
  ...
  const geodBtn = document.querySelector(".geolocation");
  let socket = new WebSocket(wsURL);

  socket.onopen = () => {
    infoOutput.innerHTML = `<p>Соединение установлено</p><svg class="svg-connect" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-left-right" viewBox="0 0 16 16">
  ...
  geodBtn.addEventListener("click", getLocation);
  ...
  function locationSuccess(data) {
    // let link = `https://www.openstreetmap.org/?pt=${data.coords.longitude},${data.coords.latitude}&z=1881=map `
    let link = `https://www.openstreetmap.org/#map=13/${data.coords.latitude}/${data.coords.longitude}`;
    writeToChat(`<a href="${link}" target="_blanc"><svg
                class="svg-geolocation" xmlns="http://www.w3.org/2000/svg"
                width="16"
                height="16"
                fill="currentColor"
  ...
  function locationError() {
    writeToChat(`<p>При определении местоположения произошла ошибка</p><a class="info-output" href="https://support.google.com/chrome/answer/142065?hl=ru&co=GENIE.Platform%3DDesktop&oco=0" target="_blanc">Подробнее как исправить эту проблему<svg class="svg-geolocation" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-eyeglasses" viewBox="0 0 16 16">
